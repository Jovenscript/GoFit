// --- SISTEMA SENSORIAL (ÁUDIO E VIBRAÇÃO) ---
...
// --- BANCO DE DADOS ATUALIZADO COM TIERS (CORES) ---
const foodDatabase = [
    // VERDES (Saudáveis)
    ...
    // VERMELHOS (Calóricos / Fast Food)
    { id: 22, name: "Refrigerante (Lata)", portion: "350ml", kcal: 149, p: 0, c: 37, f: 0, tier: 'red' },
    ...
    { id: 25, name: "Batata Frita (M)", portion: "100g", kcal: 312, p: 3.4, c: 41, f: 15, tier: 'red' },
    { id: 26, name: "Chocolate ao Leite", portion: "30g", kcal: 160, p: 2, c: 18, f: 9, tier: 'red' },
    { id: 27, name: "Milkshake Pequeno", portion: "300ml", kcal: 420, p: 8, c: 68, f: 14, tier: 'red' }
];
...
const workoutsDB = {
    A: {
        ...
        exercises: [
            ...
            { name: "Crucifixo Máquina", sets: "3", reps: "12-15" },
    ...
    C: {
        ...
        exercises: [
            ...
            { name: "Leg Press 45º", sets: "4", reps: "10-15" },
            { name: "Cadeira Extensora", sets: "4", reps: "15" },
            { name: "Cadeira Flexora", sets: "4", reps: "12-15" },
            { name: "Panturrilha Máquina", sets: "5", reps: "15-20" }
        ]
    }
};

let userData = JSON.parse(localStorage.getItem('healthAppUserData')) || null;
...
window.onload = () => {
    ...
    // Inicia contexto de áudio em qualquer clique na tela para burlar bloqueio de navegadores
...
function setWaterRingProgress(percent) {
    ...
    const circumference = radius * 2 * Math.PI; // Para r=50, C ≈ 314
    
    // Limita o círculo em 100% no máximo para a matemática
...
function init3DTilt() {
    const card = document.querySelector('.main-calories');
    if(!card) return;
...
function getDailyLog() {
    ...
    let log = JSON.parse(localStorage.getItem('healthAppDailyLog'));
    if (!log || log.date !== todayDate) {
        log = { date: todayDate, waterConsumed: 0, foods: [], workoutDone: { A: [], B: [], C: [] } };
        localStorage.setItem('healthAppDailyLog', JSON.stringify(log));
    }
    return log;
}
...
function saveState() {
    ...
    localStorage.setItem('healthAppDailyLog', JSON.stringify(dailyLog));
    updateDashboardUI();
    renderMeals();
}
...
function saveProfile() {
    ...
    userData = { age, weight, height, goal, tdee, waterGoal: Math.round(weight * 40), macros: { p: protTarget, c: carbTarget, f: fatTarget } };
...
function addWater(amount) {
    ...
    if (waterBefore < 100 && waterAfter >= 100) {
        triggerFeedback('success');
        fireConfetti();
        showToast("🎉 Meta de água batida!");
    } else {
        triggerFeedback('click');
        showToast(`💧 +${amount}ml registrados!`);
    }
}
...
function renderMeals() {
    ...
    mealNames.forEach((meal, index) => {
        ...
        let foodsHtml = mealFoods.length > 0 
            ? `<p class="meal-card-details">${mealFoods.map(f => f.name).join(', ')}</p>` 
            : `<p class="meal-card-details" style="color:rgba(255,255,255,0.3)">Nenhum alimento</p>`;
        ...
        card.innerHTML = `
            <div style="display:flex; justify-content:space-between;">
                ...
                <span style="font-size:14px; font-weight:800; color:var(--primary-light)">${mealKcal} kcal</span>
            </div>
...
function updateDashboardUI() {
    ...
    if (dailyLog.foods.length === 0) {
        logContainer.innerHTML = '<p style="text-align:center; color:var(--text-light); font-size:14px; padding: 25px 0;">Nada registrado ainda hoje.</p>';
    }
...
function addSelectedFood(foodId, foodName) {
    ...
    saveState();
    closeFoodModal();
    showToast(`🍽️ ${foodName} adicionado!`);
}
...
function renderWorkout(tab) {
    ...
    workoutData.exercises.forEach((ex, index) => {
        ...
        item.innerHTML = `
            <div class="ex-info">
                <h4>${ex.name}</h4>
                <p>${ex.sets} séries x ${ex.reps} reps</p>
            </div>
            <div class="ex-check ${isDone ? 'done' : ''}" onclick="toggleExercise('${tab}', ${index}, this)">
                ${isDone ? '✓' : ''}
            </div>
        `;
